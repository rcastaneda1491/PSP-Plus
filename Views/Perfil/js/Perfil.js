...
function GetDatos() {
    ...
    const url = `https://172.30.236.13:8082/api/Perfil?idUsuario=${jwt.sub}`;
...
function actualizar() {
    ...
    fechaInput.value === '' || equipoInput.value === '') {
        ...
        if (confirmar) {
            ...
            const urlActualizarUsuario = `https://172.30.236.13:8082/api/Perfil?idUsuario=${jwt.sub}&nombre=${nombresInput.value}&apellido=${apellidosInput.value}&email=${correoInput.value}&clave=${contra}&fechaNacimiento=${fechaInput.value}&idEquipo=${equipoInput.value}`;
